...
public:
	...
	Shader(const std::string filepath) {
		...
		if (!success) {
			...
			throw std::exception("vertex shader compilation error");
		};
		glGetShaderiv(fragmentShader, GL_COMPILE_STATUS, &success);
		if (!success) {
			glGetShaderInfoLog(fragmentShader, 512, NULL, infoLog);
			std::cout << "ERROR::SHADER::FRAGMENT::COMPILATION_FAILED\n" << infoLog << std::endl;
			throw std::exception("fragment shader compilation error");
		};
		id = glCreateProgram();
	...
	void setUniformVec4(const GLchar * name, glm::vec4 &color) {
		bind();
		int loc = glGetUniformLocation(id, name);
		if (loc == -1) {
			std::cout << "uniform didnt find" << std::endl;
	...
	void setUniform3m(const GLchar * name, glm::mat3 &mat) {
		bind();
		int loc = glGetUniformLocation(id, name);
		if (loc == -1) {
			std::cout << "uniform didnt find" << std::endl;;
